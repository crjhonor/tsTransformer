...
embedding_sizes = [
    (x, min(50, (x+1) // 2)) for x in cardinality
]
"""
...
class StaticDynamicFeatureRNNModel(SingleStepRNN):
    ...
    def forward(self, x: Dict):
        ...
        if cat.size(-1)>0:
            ...
            x_cat = torch.cat([emb(cat[:, :, i]) for i, emb in enumerate(self.embedding)], dim=-1)
...
# Training the model
save_model_full = Path(os.getcwd()).parent / 'assa/model/saved_weights/baseline_static_time_varying.wt'
train_model = True
...
performance_show.to_csv(Path(os.getcwd()).parent / 'assa/model/performance_metrics/baseline_static_time_varying_metric_record.csv')
pred_df.to_csv(Path(os.getcwd()).parent / 'outputs/pred_dfs/baseline_static_time_varying_metric_label_T1.csv')

print(pred_df.xs('Y0Close'))
